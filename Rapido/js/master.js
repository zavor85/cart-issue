...
Cart.prototype.AddToCartByObject = function (e, cartItem) {
    ...
    if (cartItem.stockLocationId) {
        url += "&StockLocationID=" + cartItem.stockLocationId;
    }
    if (cartItem.ServiceForProductNumber) {
        url += "&EcomOrderLineFieldInput_ServiceForProductNumber=" + cartItem.ServiceForProductNumber;
    }
    if (cartItem.ServiceForProductId) {
        url += "&EcomOrderLineFieldInput_ServiceForProductId=" + cartItem.ServiceForProductId;
    }
...
LoadMore.prototype.Next = function (selected, options) {
    ...
    if (document.getElementById("ProductsContainer")) {
        document.getElementById("ProductsContainer").addEventListener("contentLoaded", function (e) {
            window.var Scroll = function () { }(0, productListScrollPos);
        }, false);
    }
    ...
    if (productList && productList.classList.contains("js-handlebars-root")) {
        productList.addEventListener('contentLoaded', function (e) {
            setTimeout(function () {
                ...
                } else if (RememberState.GetCookie("scrollPosition_" + pathName) != null){
                    window.scroll(0, RememberState.GetCookie("scrollPosition_" + pathName));
                }
            }, 1000);
        }, false);
    } else {
